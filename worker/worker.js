export default {
  async fetch(request, env, ctx) {
    ...
    const isValidRoomNum = (roomNum) => {
      ...
      return (n >= 1 && n <= 30) || n === TEST_ROOM_NUM;
    };
    ...
    const calcExpectedAmount = (roomNum, roomData = {}) => {
      ...
      const rent = Number(roomData.prorateRent || roomData.rent || (r <= 20 ? 2500 : 3000));
      const trash = Number(roomData.trash !== undefined ? roomData.trash : 50);
    ...
    const getRoomRentValue = (roomNum, roomData = {}) => {
      ...
      const r = parseInt(roomNum, 10);
      return Number(roomData.prorateRent ?? roomData.rent ?? (r <= 20 ? 2500 : 3000)) || 0;
    };

    const getRoomTrashValue = (roomNum, roomData = {}) => {
      if (isTestRoom(roomNum)) return 0;
      return Number(roomData.trash !== undefined ? roomData.trash : 50) || 0;
    };
    ...
    const createR2Backup = async ({ backupType = 'auto-r2', reason = 'auto', billingMeta = {}, createdFrom = 'worker' } = {}) => {
      ...
      return { ok: true, backupId, objectKey, size: jsonText.length, createdAt: backup.createdAt, createdAtText: backup.createdAtText, backupType, reason };
    };

    const autoBackupBeforeImportantAction = async (reason, billingMeta = {}) => {
      try {
        return await createR2Backup({ backupType: 'auto-r2', reason, billingMeta, createdFrom: 'worker-auto' });
      } catch (err) {
        await logEvent({ level: 'warn', action: 'autoBackupToR2', message: err.message || String(err), extra: { reason } });
        return { ok: false, error: err.message || String(err), reason };
      }
    };
    ...
    const applyPaymentToRoom = ({
      ...
      const currentFullAmount = room && !room.vacant ? calcExpectedAmount(roomNum, room) : 0;
      ...
      if (remainingPayment > 0 && currentAmount > 0 && room) {
        ...
        const fullTotal = calcExpectedAmount(roomNum, room);
    ...
    if (body.action === 'manualPayment') {
      ...
      const allowedManualMethods = ['manual', 'cash', 'offline_transfer', 'bank_transfer', 'transfer', 'other'];
      ...
      const currentFullBefore = room && !room.vacant ? calcExpectedAmount(roomNum, room) : 0;
            ...
            if (event.type === 'message' && event.message?.type === 'image') {
              ...
              if (roomNum && rms[roomNum]) {
                const currentExpected = rms[roomNum].paid
                  ? 0
                  : Math.max(0, calcExpectedAmount(roomNum, rms[roomNum]) - Number(rms[roomNum].manualPaidAmount || 0));
...
async function runAutoRentReminder(env) {
  ...
  const calcTotal = (roomNum, d) => {
    ...
    const rent = d.prorateRent || (r <= 20 ? 2500 : 3000);
    return rent + elec + water + 50 + (Number(d.wifi) || 0);
  };
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    if (config.mutedRooms && config.mutedRooms[i]) {
      skippedMuted++;
      continue;
    }
